...
let ( let* ) (x : ('a, [< error ]) result) (f : 'a -> ('b, [> error ]) result) :
    ('b, error) result =
  match x with Ok v -> f v | Error e -> Error (e :> error)
...
let extract line todo_path read =
  ...
  if line < 1 || line > List.length todos then Error (`WrongLine line)
  else
...
let edit line todo_path effects =
  ...
  let* edited = effects.editor todo in
  if edited = "" || edited = todo then Ok ()
  else
    let updated =
      todos
      |> List.mapi (fun idx content ->
          if idx = line - 1 then edited else content)
    in
    let* _ = effects.write updated todo_path in
    Ok ()
...
let sort_matches projects =
  List.sort
    (fun path1 path2 ->
      ...
      if count1 <> count2 then compare count1 count2
      else
