...
let parser path args =
  ...
  | [ single ] when batch_cmd '+' single ->
      Some
        (Complete (path, list_from (drop 1 single) |> List.map int_of_string))
  | [ single ] when batch_cmd '-' single ->
      Some (Remove (path, list_from (drop 1 single) |> List.map int_of_string))
  | [ single ] when cmd ':' single ->
      Some (Edit (path, int_of_string (drop 1 single)))
  | [ single ] when cmd 'c' single ->
      Some (Commit (path, int_of_string (drop 1 single), false))
  | [ single ] when cmd '@' single ->
      Some (Doing (path, int_of_string (drop 1 single)))
...
let command_router todo_path args effects =
  let is_project project =
    String.length project > 1
    && String.get project 0 = '.'
    && String.get project 1 <> '@'
  in
...
let () =
  case "Parser" (fun test ->
      test "Echo" (fun expect ->
          expect.equal
            (parser any_todo_path [ "10" ])
            (Some (Echo (any_todo_path, 10))));
      ...
      test "List doing across projects" (fun expect ->
          expect.equal
            (parser any_todo_path [ ".@" ])
            (Some ListDoingAcrossProjects));
      ...
      test "Edit" (fun expect ->
          expect.equal
            (parser any_todo_path [ ":32" ])
            (Some (Edit (any_todo_path, 32))));
      test "Edit .todo" (fun expect ->
          expect.equal
            (parser any_todo_path [ ":" ])
            (Some (EditFile any_todo_path)));
  ...
  case "Project namespacing" (fun test ->
      ...
      let cases =
        [
          ...
          ( "Complete",
            [ "+1,2" ],
            Some (Complete ("/User/some-project/.todo", [ 1; 2 ])) );
          ( "Remove",
            [ "-5,2" ],
            Some (Remove ("/User/some-project/.todo", [ 5; 2 ])) );
          ("Edit", [ ":10" ], Some (Edit ("/User/some-project/.todo", 10)));
          ("Doing", [ "@1" ], Some (Doing ("/User/some-project/.todo", 1)));
          ( "Commit",
            [ "c1" ],
            Some (Commit ("/User/some-project/.todo", 1, false)) );
        ]
      in
      cases
