...
let make_expect description errors_ref =
  let is bool message =
    if bool then () else raise_error description message errors_ref
  in
  let equal expected actual =
    if expected = actual then () else raise_error description "" errors_ref
  in
...
let case_factory name test_fn =
  ...
  test_fn (fun description expect_fn ->
      try
        expect_fn (make_expect description errors);
        passed := !passed + 1
      with
      | Test_failed ->
          ();
      ...
      | exn ->
          errors :=
            format_error description ("Failed: " ^ Printexc.to_string exn)
            :: !errors;
          failed := !failed + 1);
...
let run_cases cases =
  cases
  |> List.iter (fun (name, passed, failed, errors) ->
      Printf.printf
        "\u{001B}[1m%-20s\u{001B}[0m | \u{001B}[92mPassed: %2d\u{001B}[0m | \
         \u{001B}[91mFailed: %2d\u{001B}[0m"
        name passed failed;
      if errors <> [] then (
        print_newline ();
...
let on_exit () =
  let total_passed, total_failed =
    List.fold_left
      (fun (acc_p, acc_f) (_, passed, failed, _) ->
        (acc_p + passed, acc_f + failed))
      (0, 0) !cases
  in
