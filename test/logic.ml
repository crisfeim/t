    ...
    |> List.iteri (fun i (read, expected) ->
      test (case_id i) (fun fn ->
        assert_list
          fn
          expected
          (list "any todo path" { (mock_effects ()) with read = (fun _ -> read) }))))
;;
      ...
      test (case_id i) (fun fn ->
        assert_str
          ...
          (add
             "any todo"
             "any todo path"
             { (mock_effects ()) with read = (fun _ -> read); write = (fun _ _ -> write) }))))
;;
      ...
      let _ =
        commit
          ...
          { (mock_effects ()) with
            read =
              (fun path ->
                if path = "any done path"
                then Ok [ "20260625 some" ]
                else Ok [ "any todo" ])
          ; write =
              (fun data path ->
                write_calls := !write_calls @ [ path, data ];
    ...
    test "uses editor output as commit message" (fun expect ->
      ...
      let _ =
        commit
          ...
          { (mock_effects ()) with
            read =
              (fun path -> if path = "any done path" then Ok [] else Ok [ "any todo" ])
          ; editor = (fun _ -> Ok "edited")
          ; commit =
              (fun msg _ ->
                commit_msg := msg;
...
let () =
  case "Projects" (fun test ->
    [ Error `FileSystem, Error `FileSystem; Ok [ "p1"; "p2" ], Ok [ "p1"; "p2" ] ]
    |> List.iteri (fun i (projects_r, expected) ->
      test (case_id i) (fun fn ->
        assert_list
          fn
          expected
          (projects { (mock_effects ()) with projects = (fun () -> projects_r) }))))
;;
  ...
  case "Sort matches" (fun test ->
    ...
    test "sort_matches: prioritizes shortest path on equal depth" (fun expect ->
      ...
      expect.equal
        ...
        (sort_matches input));
    test
      "sort_matches: falls back to alphabetical order on equal depth and length"
      (fun expect ->
