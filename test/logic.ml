  ...
  case "List" (fun test ->
      ...
      |> List.iteri (fun i (read, expected) ->
          test (case_id i) (fun fn ->
              assert_list fn expected
                (list "any todo path"
                   { (mock_effects ()) with read = (fun _ -> read) }))))
...
let () =
  case "Add" (fun test ->
      ...
      |> List.iteri (fun i (read, write, expected) ->
          test (case_id i) (fun fn ->
              assert_str fn expected
                (add "any todo" "any todo path"
                   {
                     (mock_effects ()) with
                     read = (fun _ -> read);
                     write = (fun _ _ -> write);
...
let () =
  case "Complete" (fun test ->
      ...
      |> List.iteri (fun i (read, line, write, expected) ->
          test (case_id i) (fun fn ->
              assert_str fn expected
                (complete line "any todo path" "any done path"
                   {
                     (mock_effects ()) with
                     read = (fun _ -> read);
                     write = (fun _ _ -> write);
      ...
      test "writes to done_path before updating todo_path" (fun expect ->
          ...
          let _ =
            complete 1 "any todo path" "any done path"
              {
                (mock_effects ()) with
                read =
                  (fun path ->
                    if path = "any done path" then Ok [] else Ok [ "tarea" ]);
                write =
                  (fun data path ->
                    write_calls := !write_calls @ [ (path, data) ];
                    Ok ());
...
let () =
  case "Edit" (fun test ->
      ...
      test "writes edited data" (fun expect ->
          let write_calls = ref [] in
          let _ =
            edit 1 "any todo path"
              {
                (mock_effects ()) with
                read = (fun _ -> Ok [ "any todo" ]);
                write =
                  (fun todos _ ->
                    write_calls := todos :: !write_calls;
                    Ok ());
  ...
  case "Commit" (fun test ->
      [
        ...
        ( any_repo,
          ...
          Ok (),
          Ok (),
          Error (`CommitError "Commit aborted due to empty message") );
        ( any_repo,
          Ok [ "any todo" ],
          1,
          Ok "any edition",
          Error (`CommitError "any error"),
          Ok (),
          Error (`CommitError "any error") );
        ( any_repo,
          Ok [ "any todo" ],
      ...
      |> List.iteri
           (fun i (repo, read, line, editor, commit_r, write, expected) ->
             test (case_id i) (fun fn ->
                 assert_unit fn expected
                   (commit line "any todo path" "any done path" true
                      {
                        (mock_effects ()) with
                        read = (fun _ -> read);
                        write = (fun _ _ -> write);
      ...
      test "archives todo in correct order on success" (fun expect ->
          let write_calls = ref [] in
          let _ =
            commit 1 "any todo path" "any done path" true
              {
                (mock_effects ()) with
                read =
                  (fun path ->
                    if path = "any done path" then Ok [ "20260625 some" ]
                    else Ok [ "any todo" ]);
                write =
                  (fun data path ->
                    write_calls := !write_calls @ [ (path, data) ];
            ...
            commit 1 "any todo path" "any done path" true
              {
                ...
                read =
                  (fun path ->
                    if path = "any done path" then Ok [] else Ok [ "any todo" ]);
                editor = (fun _ -> Ok "edited");
                commit =
                  (fun msg _ ->
                    commit_msg := msg;
      ...
      test "uses todo as commit message when open_editor is false"
        (fun expect ->
          let commit_msg = ref "" in
          let _ =
            commit 1 "any todo path" "any done path" false
              {
                (mock_effects ()) with
                read =
                  (fun path ->
  ...
  case "Projects" (fun test ->
      ...
      |> List.iteri (fun i (projects_r, expected) ->
          test (case_id i) (fun fn ->
              assert_list fn expected
                (projects
                   { (mock_effects ()) with projects = (fun () -> projects_r) }))))

let () =
  case "Project" (fun test ->
      [
        (Error `FileSystem, Ok [], Error `FileSystem);
...
let () =
  case "Sort matches" (fun test ->
      test "sort_matches: prioritizes shallowest path" (fun expect ->
          let input = [ "/User/nested/any-project"; "/User/any-project" ] in
          expect.equal
            [ "/User/any-project"; "/User/nested/any-project" ]
            (sort_matches input));
      test "sort_matches: prioritizes shortest path on equal depth"
        (fun expect ->
          let input =
            [ "/User/nested_longest/any-project"; "/User/short/any-project" ]
          in
          expect.equal
            [ "/User/short/any-project"; "/User/nested_longest/any-project" ]
            (sort_matches input));
      test
        "sort_matches: falls back to alphabetical order on equal depth and \
         length" (fun expect ->
          let input = [ "/User/t/any-project"; "/User/a/any-project" ] in
          expect.equal
            [ "/User/a/any-project"; "/User/t/any-project" ]
            (sort_matches input)))
